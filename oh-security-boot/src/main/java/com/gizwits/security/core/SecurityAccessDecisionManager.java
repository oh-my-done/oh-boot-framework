...
public class SecurityAccessDecisionManager implements AccessDecisionManager {
    ...
    /**
     * @param authentication   用户具有的角色权限
     * @param object
     * @param configAttributes 访问该资源所需的权限
    ...
    public void decide(Authentication authentication, Object object, Collection<ConfigAttribute> configAttributes) throws AccessDeniedException, InsufficientAuthenticationException {
        ...
        if (ANONYMOUS_USER.equalsIgnoreCase(authentication.getPrincipal().toString())) {
            return;
        }

        if (configAttributes == null) {
            throw new AccessDeniedException(ResponseCode.NO_PERMISSON.getName());
        }

        for (ConfigAttribute attribute : configAttributes) {
            String roleName = attribute.getAttribute();
            for (GrantedAuthority authority : authentication.getAuthorities()) {
                if (roleName.equals(authority.getAuthority().trim())) {
