...
public class SwaggerAutoConfiguration implements BeanFactoryAware {
    ...
    public List<Docket> createRestApi(SwaggerProperties swaggerProperties) {
        ...
        if (swaggerProperties.getDocket().size() == 0) {
            ...
            if (swaggerProperties.getEnabledSecurity()) {
        ...
        for (String groupName : swaggerProperties.getDocket().keySet()) {
            ...
            if (docketInfo.getEnabledSecurity()) {
                docket = docket.securitySchemes(groupSecuritySchemes(docketInfo));
            }
    ...
    private List<ApiKey> groupSecuritySchemes(SwaggerProperties.DocketInfo docketInfo) {
        return docketInfo.getHeader().stream().map(h -> new ApiKey(h, h, "header")).collect(Collectors.toList());
    }

    private List<ApiKey> securitySchemes() {
        return swaggerProperties().getHeader().stream().map(h -> new ApiKey(h, h, "header")).collect(Collectors.toList());
    }
