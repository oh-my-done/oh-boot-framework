...
import com.gizwits.pay.weixin.util.HttpUtils;
...
public class WxPayApi {
    ...
    public static String getsignkey(String mch_id, String partnerKey) {
        ...
        map.put("sign", WxPaySignature.sign(map, partnerKey));
    ...
    public static String getCodeUrl(String appid, String mch_id, String product_id, String partnerKey, boolean isToShortUrl) {
        ...
        String packageSign = WxPaySignature.sign(packageParams, partnerKey);
        String qrCodeUrl = WxPaySignature.replace(url, "XXXXX", packageSign, appid, mch_id, product_id, timeStamp, nonceStr);

        if (isToShortUrl) {
            String shortResult = WxPayApi.toShortUrl(WxPaySignature.buildShortUrlParasMap(appid, null, mch_id, null, qrCodeUrl, partnerKey));

            WxPayAsyncResponse wxPayAsyncResponse = XmlUtil.fromXML(shortResult, WxPayAsyncResponse.class);
            if (WxPaySignature.codeIsOK(wxPayAsyncResponse.getReturnCode())) {
                if (WxPaySignature.codeIsOK(wxPayAsyncResponse.getResultCode())) {
                    qrCodeUrl = wxPayAsyncResponse.getShortUrl();
                }
            }
        }
    ...
    public static String doPost(String url, Map<String, String> params) {
        return HttpUtils.post(url, WxPaySignature.toXml(params));
    }

    public static String doPostSSL(String url, Map<String, String> params, String certPath, String certPass) {
        return HttpUtils.postSSL(url, WxPaySignature.toXml(params), certPath, certPass);
    }
